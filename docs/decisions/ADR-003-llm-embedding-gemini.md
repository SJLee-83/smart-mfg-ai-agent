...
기재되어 있다. 또한 기획서 내부에 임베딩 모델이 **5.1(Solar) vs 5.3(OpenAI)** 로 엇갈리는 불일치가
있고(부록 A9), 이는 미확정 가정으로 남아 있었다.

재개발 시 아래 이유로 **Gemini 단일 스택**으로 전환한다.
...
> Matryoshka(MRL)를 지원해 768 / 1536 / 3072 차원 중 선택 가능하며, 우선 **768d**로 검증한다.
> 모델 현행성은 변동될 수 있으므로 **구현 착수 시 Google AI 공식 문서를 재확인**한다.
...
지원 종료 상태임을 발견하고 현행 GA 모델로 교체해 본 ADR을 확정했다. "측정·확인된 것만 채택한다"는
