...
**과장 금지(ADR-002 원칙, CLAUDE.md §2)**: 이 구조의 채택 근거는 "에이전트가 필요해서"가 아님. LLM이 자율적으로 라우팅·도구선택하지 않는 단일 턴·규칙 기반 구조이므로 외부 표기는 **"멀티에이전트"가 아니라 "조건부 라우팅 상태 머신 + 1회 재시도 엣지"**. "LLM 기반 오케스트레이션", "자가수정 루프", "동적 도구 선택", 측정 안 한 정확도·"최적화" 주장 금지.
