...
기획서 §5.3은 Vector DB로 **PostgreSQL pgvector**를 기재한다. 이전 해커톤 구현도 pgvector를 사용했다.
재개발 시 **규모·개발 속도·정직성** 기준으로 재검토한다.
...
2. **로컬 개발 속도** — pgvector는 PostgreSQL 서버·Docker가 필요하다. Chroma는 `pip install` +
   파일 기반으로 즉시 시작 가능. 검증 우선 프로젝트에서 인프라 셋업 비용을 최소화한다.
3. **포트폴리오 정직성** — 이전 레포가 pgvector를 썼으나 실제 장점(대규모·관계형 결합)을 살리지 못했다.
   재개발에서 **규모에 맞는 기술 선택을 명시적으로 보여주는 것**이 더 나은 어필이다.
