...
원칙: **pdf_loader만 I/O를 만진다.** `parse`·`tag`는 입력→출력 순수함수(파일·env·네트워크 접근 금지) → PDF 없이 인라인 문자열로 단위 테스트 가능.
...
파서는 구조를 `explanation`/`actions`/`title` 필드로, `content_type`(TROUBLESHOOTING/SAFETY)은 tagger의 메타데이터로. 서로 직교한 관심사라 한 필드명에 섞지 않는다. (3인 합의)
