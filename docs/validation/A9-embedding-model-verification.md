...
- **실제 `src/index/` 임베딩 모듈은 `google-genai`(신 SDK)로 구현**한다. 폐기된 `google-generativeai`는 `requirements.txt`에 넣지 않는다(폐기 모델/SDK를 본 구현에 올리지 않는다는 ADR-003 원칙과 동일).

## 비고 (보안·정직성)

- `GOOGLE_API_KEY` 유효 확인. 키는 `.env`(git-ignored)에만 보관, 출력·커밋 안 함. 검증 임시 스크립트·출력은 실행 후 삭제.
- 이 검증은 **"호출 가능 + 차원"**까지다. 임베딩 **품질**(검색 정확도)은 별도 측정 대상(A4 메타데이터 필터링·검색 평가와 함께) — 아직 미검증. "임베딩 동작 확인"과 "검색 정확도 검증"을 구분한다.
