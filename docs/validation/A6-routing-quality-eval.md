...
- 라우팅은 **규칙 기반**(`CODE_RE` 정규식 + 0건 판정)이며 결정적. 이 테스트셋은 입력이 **깨끗**한 편으로, A/C는 항상 `SRVO-NNN`을 정확한 형식으로 포함하고 B는 코드 토큰이 전혀 없음. 따라서 100%는 "**정상 형식 입력에서 분기가 설계대로 동작**"의 확인이지 변형 입력 견고성의 입증이 아님
- **미검증 입력**: 공백/오타 코드(`SRVO 062`, `srvo-062` 소문자), 오타, 코드가 문장 중간에 비정형으로 박힌 경우. `CODE_RE`(`SRVO-\d{3}`, 대문자·하이픈 고정)는 이들을 놓칠 수 있으며 이 실험은 그 경계를 다루지 않음
- 25개는 **소표본**이고 라벨은 설계자가 부여 - 라벨 편향 가능(EXPERIMENT_PLAN §7).

**M2 = 100%는 "재시도가 위험하다"의 직접 증거가 아니다 - 정의상 결과다.**

- 이 시스템에서 `unfiltered_fallback`은 **필터 검색이 0건일 때만** 발생. 메타데이터는 **정확 일치**이고 각 코드는 정확히 1청크이므로 필터 0건 = **요청 코드가 인덱스에 없음**과 동치
- 즉 fallback이 도는 모든 경우는 "요청 코드가 코퍼스에 부재"이므로, 비필터 재검색이 돌려주는 출처가 요청 코드와 일치하는 것은 **구조적으로 불가능**. **M2 = 100%는 측정이라기보다 정의의 귀결**
- 따라서 이 100%는 "fallback 답변 5건이 모두 품질 불량"을 뜻하지 않음. M2는 EXPERIMENT_PLAN §3·§7이 명시한 대로 **자동 프록시**일 뿐이며, 코드가 달라도 의미상 관련 알람일 가능성 존재(정성 검토 영역)

**부수 관측(별도 미측정 축)**:
- `none` 가지는 도달 0회. 비필터 검색은 컬렉션이 비어있지 않는 한 항상 최근접 이웃을 반환하므로 **현 데이터에서 `none`은 사실상 도달 불가**(컬렉션 공집합일 때만 발생)
...
- **존재하지 않는 코드 처리**: 현재는 무관한 최근접 코드로 답변하고 `unfiltered_fallback` 라벨로만 경고. "매뉴얼에 없는 코드"임을 **명시 고지**하거나 `none`으로 끊는 편이 더 안전할 수 있어 프롬프트 헤징 또는 라우팅 보강을 검증 대상으로 등록(EXPERIMENT_PLAN §5, langgraph-multiagent §6 보류 항목)
...
- 이 측정 범위는 **라우팅·출처**까지. **답변 품질**(생성문 정확성·안전성)과 **검색 정확도**(올바른 청크 회수)는 별개의 미검증 축이며, "라우팅이 의도대로 분기"와 "답변/검색이 정확"은 구분

## 7. 후속 측정 - 변형 입력 라우팅 견고성 (2026-06-07)

§4·§5에서 "M1 100%는 정상 형식 입력 한정"이라 적은 한계를 직접 측정. 도구는 `experiments/a6_routing_quality/run_robustness_eval.py`(9케이스, 검색 실호출·답변 스텁). 측정 중 **소문자 입력 버그 발견·수정**
...
**중요(정직 기록): `re.IGNORECASE` 추가만으로는 9/9 불가**(실측 확인). 소문자로 매칭해도 매칭 텍스트가 소문자 그대로면 메타데이터 필터(대문자 저장)가 빗나가 `unfiltered_fallback`으로 귀결. 따라서 (1) 정규식 IGNORECASE + 선택적 하이픈, (2) `canonical_code()`로 감지 코드를 `SRVO-NNN` 정규형으로 변환하는 두 단계를 모두 거쳐야 `filtered` 도달
...
- **오타는 여전히 미복구**(의도된 설계): `SRVO-06 2`(공백 삽입)·`SRV0-062`(O→0)는 unfiltered로 분기. 오타 교정 미구현이나 "엉뚱한 코드로 오인하지 않고 일반 검색으로 폴백"하므로 **안전한 실패**
- **별칭→코드 매핑 미구현**: `BZAL`(SRVO-062 알람 약어) 같은 별칭은 코드로 해석되지 않고 의미 검색(unfiltered)에 의존. 별칭 사전 없음
- 9케이스는 **소표본**으로 모든 변형(다국어 혼용, 코드 다중 표기 등)을 망라하지 않음
