...
이 작업은 사용자 요청에서 "멀티에이전트 구조"로 명명됐으나, **팀 합의 결론은 그 명칭을 외부·자소서에 쓰지 않는 것**이다. 근거:
...
- ❌ "자가수정 / 반복 추론 / agentic 루프" — 0건 조건의 정확히 1회 fallback. "단일 fallback 재시도"라고만.
