...
- **측정됨(A6 일부, 2026-06-07)**: **라우팅 정확도 25/25(100%)** — 25질의 클린 테스트셋에서 모두 설계 의도대로 분기([검증 기록](docs/validation/A6-routing-quality-eval.md)). 단 라우팅은 규칙 기반·결정적이고 입력이 정상 형식이라, 이 수치는 "정상 입력에서 분기가 설계대로 동작"을 뜻하지 변형 입력(공백/소문자/오타 코드) 견고성을 입증하진 않는다. 비필터 재시도(`unfiltered_fallback`) 경로의 출처 코드 불일치율은 5/5(100%)였으나, fallback은 '인덱스에 없는 코드'에서만 발생하므로 이 100%는 측정이라기보다 **정의상 결과**다(자세한 단서는 검증 기록 §4).
- **변형 입력 라우팅 견고성(2026-06-07)**: 9케이스 측정 중 소문자 입력 버그 발견·수정(`CODE_RE`에 `IGNORECASE`+하이픈 선택 + 정규화) → **9/9**([검증 기록 §7](docs/validation/A6-routing-quality-eval.md)). 단 오타 교정·별칭(`BZAL`)→코드 매핑은 미구현(의도된 안전 폴백).
- **여전히 미검증(후속 과제)**: 답변 품질 정량 평가, 비필터 재시도 답변이 실제로 오도하는지(정성), 검색 정확도(올바른 청크 회수), LLM/임베딩 모델 비교(A6 나머지). 정량 수치는 측정 후에만 주장한다.
