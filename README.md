...
DACON 스마트 제조 AI 해커톤 **본선 진출 결과물의 개념을 계승**해, 프로덕션 수준으로 **새로 구현**한 프로젝트입니다(기존 코드는 가져오지 않았습니다). 핵심 원칙은 **검증 우선**입니다:
...
기획서의 "Cross-Reference RAG"는 실측에서 근거 데이터가 희소함을 확인하고 **마커 기반 청킹 + 메타데이터 필터링**으로 재정의했습니다([ADR-002](docs/decisions/ADR-002-cross-reference-rag-redefinition.md)) — 검증으로 가정을 기각·수정한 사례입니다.

---

## 정직성 고지

포트폴리오로서 **구현·검증한 것과 아직 측정하지 않은 것을 구분**합니다.

- 이 시스템은 **규칙 기반 조건부 라우팅 상태 머신**입니다. LLM이 라우팅하지 않으며(라우팅은 정규식+0건 판정), 단일 턴·무상태입니다. "멀티에이전트/자율 에이전트"가 아닙니다.
- **측정됨(A6 일부, 2026-06-07)**: **라우팅 정확도 25/25(100%)** — 25질의 클린 테스트셋에서 모두 설계 의도대로 분기했습니다([검증 기록](docs/validation/A6-routing-quality-eval.md)). 단 라우팅은 규칙 기반·결정적이고 입력이 정상 형식이라, 이 수치는 "정상 입력에서 분기가 설계대로 동작"함을 뜻하며 변형 입력(공백/소문자/오타 코드) 견고성을 입증하지는 않습니다. 비필터 재시도(`unfiltered_fallback`) 경로의 출처 코드 불일치율은 5/5(100%)였으나, fallback은 '인덱스에 없는 코드'에서만 발생하므로 이 100%는 측정이라기보다 **정의상 결과**입니다(자세한 단서는 검증 기록 §4).
- **변형 입력 라우팅 견고성(2026-06-07)**: 9케이스 측정 중 소문자 입력 버그를 발견·수정(`CODE_RE`에 `IGNORECASE`+하이픈 선택 + 정규화)해 **9/9**가 되었습니다([검증 기록 §7](docs/validation/A6-routing-quality-eval.md)). 단 오타 교정·별칭(`BZAL`)→코드 매핑은 미구현입니다(의도된 안전 폴백).
- **여전히 미검증(후속 과제)**: 답변 품질 정량 평가, 비필터 재시도 답변이 실제로 오도하는지(정성), 검색 정확도(올바른 청크 회수), LLM/임베딩 모델 비교(A6 나머지). 정량 수치는 측정 후에만 주장합니다.
- 비필터 재시도로 얻은 답변은 `retrieval_mode="unfiltered_fallback"`로 표기되며, 요청 코드와 다른 출처일 수 있으니 `sources`의 `error_code`로 교차확인하세요.
